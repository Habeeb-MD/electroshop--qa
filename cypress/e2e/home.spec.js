describe('Home Page', () => {
    ...
    it('shows the hero section with correct content', () => {
        cy.get('h1').should('contain', 'Welcome to ElectroShop');
        cy.get('p').should('contain', 'Discover the latest in electronics and gadgets');
        cy.get('p').should('contain', 'Shop our wide range of products and stay ahead with cutting-edge technology.');
    });
    ...
    it('displays the featured products section', () => {
        ...
        cy.get('[class="MuiGrid-root MuiGrid-container MuiGrid-spacing-xs-3 css-1h77wgb"]').should('exist');
    });

    it('renders featured products', () => {
        cy.get('[class="MuiGrid-root MuiGrid-container MuiGrid-spacing-xs-3 css-1h77wgb"]').within(() => {
            cy.get('[class^="MuiPaper-root MuiPaper-elevation MuiPaper-rounded"]').should('have.length', 6);
        });
    });

    it('allows navigation to a product detail page', () => {
        cy.get('[class="MuiGrid-root MuiGrid-container MuiGrid-spacing-xs-3 css-1h77wgb"]').within(() => {
            cy.get('[class^="MuiPaper-root MuiPaper-elevation MuiPaper-rounded"]').first().click();
        });
